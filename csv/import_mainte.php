...
if ( $mode == 'ins' ) {
	...
	case UPLOAD_ERR_OK:
		if ($id=='') {
			...
			if ($state==3) return_error("不正なアクセスです。");
		//既存データを削除
			$where = "hid=$id";
			if ($add=='') {
				$where0 = $where;
				if ($ssid<>-1) $where0 .= " and ssid=$ssid";
				$sql = "DELETE FROM $table WHERE $where0";
...
//文字コードの変換
			...
			foreach ($header as $f_jp) {
				...
				if ($skip_ss and $f=='ssid') continue;
...
<?php
if ( $mode == 'ins' ) {
	echo '<a href="javascript:window.opener.form1.submit();window.close()"><b><font size="2">更新して閉じる</font></b></a>';
} else {
	echo '<a href="javascript:window.close()"><b><font size="2">閉じる</font></b></a>';
}
?>
<br><br><br>
