...
function db_insert($conn, $table, $fields, $types, $values, $admin_mode=true)
{
    if (!$admin_mode)
    {
        $set_fields = "company";
...
function db_update($conn, $table, $fields, $types, $values, $where, $admin_mode=ture)
{
    for ($i = 0; $i < count($types); $i++)
    {
        if ($sets <> '')
    ...
    if (!$admin_mode)
    {
        if ($where <> '')
            $where = "($where) and ";
        $where .= "(company=0 or company=" . db_value($_SESSION['company'], "int") . ")";
    }
    $sql = "update `$table` set $sets where $where";
    db_exec($conn, $sql);
}

function db_delete($conn, $table, $where, $admin_mode=true)
{
    if (!$admin_mode)
    {
