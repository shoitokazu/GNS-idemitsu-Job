...
if (is_array($list)) {
	foreach ($list as $key => $values) {
		...
		if ($dels[$key]=='1') {
			...
			$price = db_value($values[$fno['price']], "int");
			$num = db_value($values[$fno['num']], "int");
			$rate = db_value($values[$fno['rate']], "int");
			$values[$fno['amount']] = round($price * $num * $rate / 100);
...
if ($addnew<>"") {
	...
	$sql1 = "INSERT INTO mainte_d (hid,sort,ssid) ";
	$sql2 = "SELECT hid,max(sort)+1,".db_value($ssid, "int")." FROM mainte_d WHERE hid=$mid GROUP BY hid";
	$sql3 = "VALUES ($mid,1,".db_value($ssid, "int").")";
	if (db_fetch_row($rs)) {
		$sql = $sql1.$sql2;
	} else {
		$sql = $sql1.$sql3;
